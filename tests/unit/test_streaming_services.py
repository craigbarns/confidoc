...
Vérifie que les générateurs stream_mistral_response et stream_kimi_response
...
# ── Tests stream_kimi_response ─────────────────────────────────────────

class TestStreamKimiResponse:

    @pytest.mark.asyncio
    async def test_disabled_yields_message(self):
        """Si KIMI_ENABLED=False, le générateur yielde un message."""
        from app.services.kimi_service import stream_kimi_response

        with patch("app.services.kimi_service.get_settings") as mock_settings:
            s = MagicMock()
            s.KIMI_ENABLED = False
            s.KIMI_API_KEY = ""
            mock_settings.return_value = s

            chunks = await collect_chunks(stream_kimi_response("test"))
            assert len(chunks) == 1

    @pytest.mark.asyncio
    async def test_api_401_yields_error_not_raises(self):
        """Une réponse 401 de Kimi ne doit PAS lever d'exception."""
        from app.services.kimi_service import stream_kimi_response
        ...
        with patch("app.services.kimi_service.get_settings") as mock_settings, \
             patch("app.services.kimi_service.httpx.AsyncClient", return_value=mock_client_ctx):
            s = MagicMock()
            s.KIMI_ENABLED = True
            s.KIMI_API_KEY = "test-key"
            s.KIMI_BASE_URL = "https://api.moonshot.ai/v1"
            s.KIMI_MODEL = "kimi-k2.5"
            s.KIMI_TIMEOUT_SECONDS = 90
            mock_settings.return_value = s
    ...
    @pytest.mark.asyncio
    async def test_timeout_yields_error_not_raises(self):
        """Un timeout Kimi ne doit PAS lever d'exception."""
        from app.services.kimi_service import stream_kimi_response
        ...
        assert len(chunks) == 1
        assert "erreur" in chunks[0].lower() or "délai" in chunks[0].lower()

    @pytest.mark.asyncio
    async def test_successful_stream_yields_chunks(self):
        """Un stream Kimi 200 OK yielde les chunks."""
        from app.services.kimi_service import stream_kimi_response

        sse_lines = [
            'data: {"choices": [{"delta": {"content": "Réponse"}}]}',
            'data: {"choices": [{"delta": {"content": " IA"}}]}',
            "data: [DONE]",
        ]
