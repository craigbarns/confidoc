...
code="$(http_json "GET" "$BASE_URL/api/v1/documents/$DOCUMENT_ID/export-structured-dataset?doc_type=$DOC_TYPE" "$STRUCTURED_JSON" "$TOKEN")"
if [[ "$code" != "200" ]]; then
...
ROUTING_SELECTED="$(json_get "$STRUCTURED_JSON" "detected_doc_type")"
STRUCTURED_DOC_TYPE="$(json_get "$STRUCTURED_JSON" "doc_type")"
...
if [[ "$COMPACT_MODE" == "1" ]]; then
  echo ""
  echo "PASS"
  echo "routing_requested: ${DOC_TYPE:-unknown}"
  echo "routing_selected: ${ROUTING_SELECTED:-unknown}"
  echo "extractor_selected: ${EXTRACTOR_SELECTED:-unknown}"
  echo "doc_type: ${STRUCTURED_DOC_TYPE:-unknown}"
  ...
  echo "routing_selected: ${ROUTING_SELECTED:-unknown}"
  echo "extractor_selected: ${EXTRACTOR_SELECTED:-unknown}"
  echo "structured_doc_type: ${STRUCTURED_DOC_TYPE:-unknown}"
  ...
  echo "routing_runner_up: ${ROUTING_RUNNER_UP:-unknown}"
