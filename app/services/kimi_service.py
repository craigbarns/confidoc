...
async def generate_summary_with_kimi(
    ...
    *,
    ...
    base_prompt = (
        ...
        "- Ne jamais vérifier ou suggérer que capitaux_propres + resultat_exercice = total_passif\n"
        "- Ne jamais signaler d'incohérence arithmétique, comptable ou patrimoniale sauf si elle apparaît explicitement dans quality_flags\n"
        "- Ne jamais écrire qu'un montant est incohérent, anormal, erroné ou contradictoire sans signal backend explicite\n"
        "- Le resultat_exercice peut déjà être inclus dans capitaux_propres : ne jamais les additionner ni les opposer\n"
        "- Les seules alertes comptables autorisées sont celles listées dans quality_flags backend\n"
        "\n"
        "RÈGLE SUR LES CHAMPS SUSPECTS:\n"
        "- Si un champ est présent dans suspicious_fields, le présenter uniquement comme 'à vérifier', 'issu d'un fallback' ou 'calculé'\n"
        "- Ne jamais présenter un champ de suspicious_fields comme confirmé, validé ou certain\n"
        "- Si suspicious_fields est vide, ne pas inventer de doute supplémentaire\n"
        "\n"
        "RÈGLE SUR LA CONFIANCE:\n"
        "- confiance_globale doit refléter uniquement les signaux backend (ready_for_ai, ready_for_ai_core, needs_review, suspicious_fields, quality_flags, coverage_ratio)\n"
        "- Ne jamais attribuer une confiance élevée si ready_for_ai=false\n"
        "- Ne jamais attribuer une confiance non nulle si le backend indique un document bloqué\n"
        "\n"
...
async def stream_kimi_response(
    ...
    *,
    ...
    try:
        async with httpx.AsyncClient(timeout=float(settings.KIMI_TIMEOUT_SECONDS)) as client:
            async with client.stream(
                ...
                if resp.status_code != 200:
                    ...
                    yield f"[Erreur Kimi {resp.status_code}: {error_body.decode(errors='replace')[:200]}]"
                    return
        ...
        yield "[Erreur: délai d'attente Kimi dépassé]"
    except Exception as exc:
        yield f"[Erreur Kimi: {exc}]"
